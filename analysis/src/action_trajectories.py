...
def compare_trajectories(intv = 0, seed = 0, agent_name = 'cnn'): 
    dir_path = "results/action_trajectories/{}/{}/{}/".format(
...
def plot_trajectories_seed(): 
    intv_list = [np.random.choice(np.arange(36)), np.random.choice(np.arange(36,65)), np.random.choice(np.arange(65,75)), np.random.choice(np.arange(75,87))]
    intv_labels = ['drop_enemy','shift_shield','shift_agent','drop_row_col']
    intv_label_names = ['drop_enemy_'+str(intv_list[0]), 'shift_shield_'+str(intv_list[1]), 'shift_agent_'+str(intv_list[2]), 'drop_row_col_'+str(intv_list[3])]
...
def plot_action(seed, intv, agent): 
    ...
    plt.ylim([0, 500])
    plt.title("Agent: "+str(agent) + " Intervention: "+str(intv_name[intv])+ " Seed: "+str(seed))
...
def action_freq_intv(int_name, agent_name):
    ...
    for itemset in a_v_s_list: 
        sums.update(itemset)
        counters.update(itemset.keys())
    actions_vanilla_summary = {x : round(float(sums[x])/counters[x],2) for x in sums.keys()}
